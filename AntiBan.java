...
public final class AntiBan {
    ...
    public static void randomBankAfk() {
    	int a = General.random(1, 20);
    	if (a == 1) {
    		debug("Leaving screen, afking 2-4 seconds");
    		getABCUtil().leaveGame();
    		General.sleep(2000, 4000);
    	}
    	if (a == 10) {
    		debug("Leaving screen, afking 10-20 seconds");
    		getABCUtil().leaveGame();
    		General.sleep(10142, 20231);

    	}
    	if (a == 17) {
    		debug("rotating camera and sleeping 5.5-12.5 seconds");
    		getABCUtil().rotateCamera();
    		General.sleep(5500, 12500);
    	}
    }
    
    public static void randomAfk() {
    	int a = General.random(1, 30);
    	if (a == 2) {
    		debug("Checked XP and sleeping for 1.5-2.4 seconds");
    		getABCUtil().checkXP();
    		General.sleep(1500, 2295);
    	}
    	if (a == 7) {
    	...
    	if (a == 16) {
    		debug("Picking up and moving mouse");
    		getABCUtil().pickupMouse();
    		getABCUtil().moveMouse();
    	}
    	if (a == 23) {
    		debug("Leaving game and sleep 2.3-4.4 seconds");
    		getABCUtil().leaveGame();
    		General.sleep(2300, 4400);
    	}
    	if (a == 30) {
    		debug("Right clicking and examining entity and sleep 1.2-2.2 sec");
    		getABCUtil().rightClick();
    		getABCUtil().examineEntity();
    		General.sleep(1200, 2130);
    	}
    }
            ...
            if (name != null) {
                ...
                if (next != null)
                    if (!Entities07.isHovering(next.getModel()) && Clicking.hover(next))
                        Timing07.waitCondition(() -> Entities07.isHovering(next.getModel()), 500);
            }
        }
    } */
